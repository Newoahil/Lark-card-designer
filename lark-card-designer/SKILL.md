...
1. Identify the input data structure and constraints:
   - data_type: KPI/Metrics, Resource/Permission (Agent), Time Series, Table/Rows, Document/Article, Alert/Log, Process/Task.
   - intent: Decision-making, Status Sync, Task Execution, Knowledge Transfer.
   - audience: Management (Result-oriented), Operations (Action-oriented), Technical (Detail-oriented).
2. Choose a card pattern based on the data's "Decision Center":
   - If Permission/Agent: Center on "Scope & Risk".
   - If Report/KPI: Center on "Trend & Benchmark".
   - If Article/News: Center on "Summary & Triage".
3. Apply structural density: 
   - High-density for professional reports (ensure order, not just brevity).
   - Low-density for alerts/approvals (ensure focus).
4. If a missing variable would materially change... (rest is same)
...
5. Attach visual/status rules. Header color and emphasis must express status, risk, priority, or action focus.
6. Add interaction rules only when the reader needs to decide, approve, select, refresh, or give feedback.
7. Output a Markdown explanation followed by a stable structured decision block.
8. Finish with design red lines and a validation checklist.
