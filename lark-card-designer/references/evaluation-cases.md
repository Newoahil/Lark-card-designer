...
10. Check that action cards include button layout, disabled/loading/final states, and audit feedback.
11. Check that `structure_sketch` is labeled as a design handoff sketch only, not production-sendable Feishu JSON.
12. Check that `design_red_lines` names the main failure modes for this scenario, not generic advice only.
